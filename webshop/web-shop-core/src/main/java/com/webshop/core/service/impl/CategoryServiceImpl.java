...
public class CategoryServiceImpl implements CategoryService
{
   ...
   public String createCategory(Category category)
   {
      ...
      if (categoryExistsByCategoryName(category).equalsIgnoreCase(Constants.CATEGORY_ALREADY_EXISTS))
      {
   ...
   /**
    * This method is a util method which does business validation for the existence of the category
    * 
    * @param Category
    * @return String
    */
   public String categoryExistsByCategoryName(Category category)
   {
      ...
      {
         result = categoryDAO.getCategoryByCategoryName(category.getCategoryName());
         if (result != null)
         {
            return Constants.CATEGORY_ALREADY_EXISTS;
         }
   ...
   public List<Category> searchCategoriesByCriteria(Category category)
   {
      ...
      try
      {
         ...
         Category result = null;
         if (StringUtils.isNotEmpty(category.getCategoryName()) && StringUtils.isEmpty(category.getCategoryDescription()))
         {
            result = categoryDAO.getCategoryByCategoryName(category.getCategoryName());
            if (result != null)
            {
         ...
         else if (StringUtils.isEmpty(category.getCategoryName()) && StringUtils.isNotEmpty(category.getCategoryDescription()))
         {
            result = categoryDAO.getCategoryByCategoryDescription(category.getCategoryDescription());
            categoryList.add(result);
         }
         else
         {
