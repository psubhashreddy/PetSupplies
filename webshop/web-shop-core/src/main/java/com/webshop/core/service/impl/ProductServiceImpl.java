...
public class ProductServiceImpl implements ProductService {
	...
	public String createProduct(Product product) {
		...
		if (productExistsByProductCode(product).equalsIgnoreCase(
				Constants.PRODUCT_ALREADY_EXISTS)
				|| productExistsByProductName(product).equalsIgnoreCase(
						Constants.PRODUCT_ALREADY_EXISTS)) {
	...
	/**
	 * This method is a utiltiy for validating the product existence by product
	 * code
	 * 
	 * @param product
	 * @return String
	...
	public String productExistsByProductCode(Product product) {
		...
		if (product != null && StringUtils.isNotEmpty(product.getProductCode())) {
			result = productDAO.getProductbyProductCode(product
					.getProductCode());
			if (result != null) {
	...
	/**
	 * This method is a utlity method for validating the product existence by
	 * product name
	 * 
	 * @param product
	 * @return String
	...
	public String productExistsByProductName(Product product) {
		...
		if (product != null && StringUtils.isNotEmpty(product.getProductName())) {
			result = productDAO.getProductByProductName(product
					.getProductName());
			if (result != null) {
				returnStr = Constants.PRODUCT_ALREADY_EXISTS;
			}
		...
		try {
			...
			if (StringUtils.isNotEmpty(product.getProductCode())
				...
				if (result != null) {
					...
					&& StringUtils.isNotEmpty(product.getProductName())) {
				result = productDAO.getProductByProductName(product
						.getProductName());
				if (result != null) {
					productList.add(result);
