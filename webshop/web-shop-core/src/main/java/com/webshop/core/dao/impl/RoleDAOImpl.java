...
/**
 * This class is the dao implementation class interacting with the database for
 * all crud operations
 * 
 * @author speddyre
 * @date 3rd June 2015
...
public class RoleDAOImpl implements RoleDAO {
	...
	/**
	 * This method is for updating the role details with the modified once
	 * (non-Javadoc)
	 * 
	...
	public Role getRoleByRoleId(int roleId) {
		...
		try {
			Query query = em.createNamedQuery("findRoleById", Role.class)
					.setParameter(Constants.ROLE_ID, roleId);
	...
	public Role getRoleByRoleName(String roleName) {
		...
		try {
			Query query = em.createNamedQuery("findRoleByRoleName", Role.class)
					.setParameter(Constants.ROLE_NAME, roleName);
			return (Role) query.getSingleResult();
		} catch (Exception e) {
	...
	public List<Role> getRoleByRoleDescription(String roleDescrption) {
		...
		try {
			Query query = em.createNamedQuery("findRoleByRoleDescription",
					Role.class).setParameter(
							Constants.ROLE_DESC,
					Constants.PERCENTAGE + roleDescrption
							+ Constants.PERCENTAGE);
			return (List<Role>) query.getResultList();
		} catch (Exception e) {
			logger.log(Level.SEVERE,
					"Exception in Searching Role By Description", e);
	...
	public List<Role> getRolesByCriteria(Role role) {
		...
		Query query = em
				.createNamedQuery("findRoleByCriteria", Role.class)
				.setParameter(Constants.ROLE_NAME, role.getRoleName())
				.setParameter(
						Constants.ROLE_DESC,
						Constants.PERCENTAGE + role.getRoleDesc()
								+ Constants.PERCENTAGE);
		return (List<Role>) query.getResultList();
	}

	/**
	 * This method is for getting the role id of the user role for the new user
	 * creation.
	 * 
	 * @return int
	 */
	public Role getUserRole() {
		try {
			...
			logger.log(Level.SEVERE,
					"Exception in Searching Role By User Role", e);
			return null;
		}
	}
}
